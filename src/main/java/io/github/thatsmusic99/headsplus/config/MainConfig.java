...
public class MainConfig extends HPConfig {
    ...
    public void loadDefaults() {
        setTitle(new Title().withWidth(100)
                ...
                .addLine("If there's ANY part of the config you can't find or don't understand, please bring it up in" +
                        " Discord:")
                ...
                .addLine("")
                .addLine("Got any other feedback? Let me know or leave a review. Don't report bugs in reviews or I " +
                        "will send the screaming goat gods your way.")
                .addLine("")
        ...
        addDefault("sell-heads", true, "Whether or not players are able to sell heads.\n" +
                ...
                "To troubleshoot this, please make sure that Economy in /vault-info isn't null. If it is, you don't " +
                "have an economy plugin.\n" +
        ...
        addDefault("leaderboards", true, "Whether or not people can view leaderboards for the plugin that list up the" +
        ...
        addDefault("masks", true, "Whether or not masks should be enabled or not.\nMasks are heads which can be worn " +
                "and apply a special effect upon the person who wears it." +
        ...
        addDefault("block-pickup", true, "Whether or not creative mode players are able to retreive heads using the " +
                "pick block button.");
        ...
        addDefault("thresholds.common", 100,
                "The maximum chance needed for the looting enchantment to increase the head drop number.\n" +
                        "For example, heads with a 50% chance of dropping will drop extra heads depending on the " +
                        "looting level.\n" +
                        "Every 100% added guarantees a new head, but the extra 50% - the remaining head - is decided " +
                        "by random chance.\n" +
        ...
        addDefault("thresholds.uncommon", 20,
                "The minimum chance needed for the looting enchantment to bump up the chance of the head dropping.\n" +
                        "In this example, a head with a 10% chance is changed to 50% with level 1 looting, 66% with " +
                        "level 2 and 75%  with level 3.\n" +
                        "Equation used: chance = level / (level + 1)");
        addDefault("thresholds.rare", 5, "The minimum chance for the looting enchantment to softly bump the chance of" +
                " a head dropping.\n" +
                "Equation used: chance *= level when chance is below 1, chance += level when chance is above or equal" +
                " to 1");
        ...
        addDefault("adjust-price-according-to-balance", false,
                "Whether or not to adjust the price of the head according to either the victim's or killer's balance " +
                        "(see below).");
        addDefault("use-killer-balance", false, "Whether to use the killer's balance or victim's balance when the " +
        ...
        addDefault("stop-placement-of-sellable-heads", false, "Whether or not players should be able to place heads " +
                "that can be sold.\n" +
                "Heads lose their metadata when placed on the ground, so this purely serves as an option to prevent " +
                "that.");
        ...
        addDefault("whitelist-worlds", false, "Whether or not the list below should be treated as a whitelist.\n" +
                ...
                ".\n" +
                "If this is enabled, on the other hand, any worlds not in the lists will not have the respected event" +
                " occur in them.");
        ...
        addDefault("crafting-list", new ArrayList<>(),
                "Worlds in which players cannot craft heads. The heads will still appear, but the player cannot pick " +
                        "them up.");
        ...
        addDefault("stats-collection", new ArrayList<>(), "Worlds in which players cannot have their HP statistics " +
                "increased.");
        addDefault("blocked-heads", new ArrayList<>(), "Heads which players cannot access using /head.\n" +
                "For example, if a player tries to do /head Thatsmusic99 but the name is included in this list, they " +
                "cannot access the head.\n" +
        ...
        addDefault("enable-autograb", false, "Enables the autograbber feature.\n" +
                "This grabs the texture of a player's head when they join and add it to the local head storage cache " +
                "for HeadsPlus.");
        addDefault("add-grabbed-heads-to-selector", true, "Whether or not player heads fetched through the " +
                "autograbber should be put in the /heads selector.");
        addDefault("autograb-section", "players", "The section in the /heads selector that grabbed heads get placed " +
                "in if the option above is enabled.");
        addDefault("autograb-display-name", "&8[&6{player}&8]", "The display name given to the head when it is " +
                "autograbbed.");
        addDefault("autograb-price", "default", "The default price set for autograbbed heads. Use \"default\" to use " +
                "the default price used for all heads.");
        ...
        addSection("Levels");
        addDefault("add-boss-bars", true, "Whether or not boss bars should be displayed to show the progress of a " +
                "player's level.");
        addDefault("boss-bar-color", "RED", "The colour of the bossbar.\n" +
                "See https://papermc.io/javadocs/paper/1.17/org/bukkit/boss/BarColor.html for a list of possible " +
                "colours.");
        ...
        addDefault("multiple-level-ups", false, "Whether or not multiple level-ups should occur at once.\n" +
                "If disabled for example, a player levelling up from A to C will only get the broadcast for level C, " +
                "but all rewards in between.\n" +
                "If enabled, there will be broadcasts for levelling up to B and C despite the player's new level " +
                "being level C.");
        ...
        addDefault("locale", "en_us", "The localisation used globally within the plugin.\n" +
                "See the locale folder for viable options. To select a language, write its file name - without the " +
                ".yml - in here.");
        ...
        addDefault("default-permissions", Lists.newArrayList("headsplus.craft.*",
                ...
                "The list of permissions users should have by default.\n" +
                        "It is not recommended to rely on this alone, but use this with a permissions plugin like " +
                        "LuckPerms.\n" +
        ...
        addDefault("suppress-messages-during-search", false, "Whether or not you are able to receive messages when " +
    ...
    public void moveToNew() {
        ...
        if (whitelist && getBoolean("whitelist.worlds.enabled", false)) {
            worlds = getList("whitelist.worlds.list");
        } else if (!whitelist && getBoolean("blacklist.worlds.enabled", false)) {
            worlds = getList("blacklist.worlds.list");
        }
        if (!worlds.isEmpty()) {
    ...
    public class MainFeatures {
        public final boolean SELL_HEADS = getBoolean("sell-heads");
        public final boolean MOB_DROPS = getBoolean("mob-drops");
        public final boolean PLAYER_DROPS = getBoolean("player-drops");
        public final boolean ENABLE_CRAFTING = getBoolean("enable-crafting");
        public final boolean HEADS_SELECTOR = getBoolean("heads-selector");
        public final boolean CHALLENGES = getBoolean("challenges");
        public final boolean LEADERBOARDS = getBoolean("leaderboards");
        public final boolean LEVELS = getBoolean("levels");
        public final boolean MASKS = getBoolean("masks");
        public final boolean INTERACTIONS = getBoolean("interactions");
        public final boolean BLOCK_PICKUP = getBoolean("block-pickup");
    }

    public class MySQL {
        public final boolean ENABLE_MYSQL = getBoolean("enable-mysql");
        public final String MYSQL_HOST = getString("mysql-host");
        public final String MYSQL_DATABASE = getString("mysql-database");
        public final String MYSQL_USERNAME = getString("mysql-username");
        public final String MYSQL_PASSWORD = getString("mysql-password");
        public final int MYSQL_PORT = getInteger("mysql-port");
    }

    public class MobDrops {
        public final List<String> BLOCKED_SPAWN_CAUSES = getStringList("blocked-spawn-causes");
        public final List<String> ENTITIES_NEEDING_KILLER = getStringList("entities-needing-killer");
        public final List<String> LOOTING_IGNORED = getStringList("looting-ignored");
        public final boolean NEEDS_KILLER = getBoolean("needs-killer");
        public final boolean ENABLE_LOOTING = getBoolean("enable-looting");
        public final boolean DISABLE_FOR_MYTHIC_MOBS = getBoolean("disable-for-mythic-mobs");
        public final double DEFAULT_DROP_CHANCE = getDouble("default-drop-chance");
        public final double DEFAULT_PRICE = getDouble("default-head-price");
        public final long DEFAULT_XP_GAINED = getLong("default-xp-gained");

    }

    public class PlayerDrops {
        public final List<String> PLAYER_HEAD_DEATH_MESSAGES = getStringList("player-head-death-messages");
    ...
    public class Masks {
        public final int CHECK_INTERVAL = getInteger("check-interval");
        public final int RESET_INTERVAL = getInteger("reset-after-x-intervals");
        public final int EFFECT_LENGTH = getInteger("effect-length");
    }

    public class Autograbber {
        public final boolean ENABLE_AUTOGRABBER = getBoolean("enable-autograb");
        public final boolean ADD_GRABBED_HEADS = getBoolean("add-grabbed-heads-to-selector");
        public final String SECTION = getString("autograb-section");
        public final String DISPLAY_NAME = getString("autograb-display-name");
        public final double PRICE = getDouble("autograb-price", -1);
    }

    public class Challenges {
        public final boolean BROADCAST_CHALLENGE_COMPLETE = getBoolean("broadcast-challenge-complete");
    }

    public class Levels {
        public final boolean ENABLE_BOSS_BARS = getBoolean("add-boss-bars");
        public final boolean BROADCAST_LEVEL_UP = getBoolean("broadcast-level-up");
        public final boolean MULTIPLE_LEVEL_UPS = getBoolean("multiple-level-ups");
        public final String BOSS_BAR_COLOR = getString("boss-bar-color");
        public final String BOSS_BAR_TITLE = getString("boss-bar-title");
        public final int BOSS_BAR_LIFETIME = getInteger("boss-bar-lifetime");
    }
    ...
    public class Localisation {
        ...
        public final boolean SMART_LOCALE = getBoolean("smart-locale");
        public final boolean USE_TELLRAW = getBoolean("use-tellraw");
    }

    public class HeadsSelector {
        public final double DEFAULT_PRICE = getDouble("default-selector-head-price");
        public final ConfigSection PER_WORLD_PRICES = getConfigSection("per-world-prices");
    }

    public class Updates {
        public final boolean CHECK_FOR_UPDATES = getBoolean("check-for-updates");
        public final boolean NOTIFY_ADMINS = getBoolean("notify-admins-about-updates");

    }

    public class Miscellaneous {
        public final boolean DEBUG = getBoolean("debug");
        public final boolean ALLOW_NEGATIVE_XP = getBoolean("allow-negative-xp");
        public final boolean SMITE_PLAYER = getBoolean("smite-player");
        public final boolean SUPPRESS_GUI_WARNINGS = getBoolean("suppress-gui-warnings");
        public final boolean SUPPRESS_MESSAGES_DURING_SEARCH = getBoolean("suppress-messages-during-search");
    }
}
